(function(){
  ...
  function bind(){
    ...
    $('categoryFilter').onchange = () => { selectedCategory = $('categoryFilter').value || null; articleListOpen = !!selectedCategory; renderSearch(); };
    $('showAllArticlesBtn').onclick = () => { articleListOpen = !articleListOpen; renderSearch(); };
    $('articleSort').onchange = () => { renderSearch(); };
  ...
  function openOffer(offer){ current = JSON.parse(JSON.stringify(offer)); articleListOpen = false; selectedCategory = null; show('editView'); populateForm(); recalc(); renderItems(); refreshPriceLists(); $('articleSearch').value=''; if($('categoryFilter')) $('categoryFilter').value=''; $('searchResults').innerHTML=''; setTimeout(()=> $('articleSearch').focus(), 150); }
  ...
  function renderSearch(){
    ...
    let found = articles.filter(a=>{
      if(selectedCategory && articleCategory(a) !== selectedCategory) return false;
      if(showAll || selectedCategory) {
        if(!terms.length) return true;
      }
      const hay=`${a.nr} ${a.name} ${articleCategory(a)} ${articleMeasure(a)}`.toLowerCase();
    ...
    if(selectedCategory){
      count.innerHTML = `<button type="button" class="category-back">Filter entfernen</button><span><strong>${esc(selectedCategory)}</strong> • ${found.length} Artikel</span>`;
      count.querySelector('.category-back').onclick = () => { selectedCategory = null; if($('categoryFilter')) $('categoryFilter').value=''; renderSearch(); };
    }else{
    ...
    shown.forEach(a=>{
      ...
      div.innerHTML=`<div class="result-main"><strong class="result-title">${esc(a.name)}</strong><div class="result-meta"><span class="pill">Nr. ${esc(a.nr)}</span>${measure?`<span class="pill">Maß ${esc(measure)}</span>`:''}${articleCategory(a)?`<span class="pill">${esc(articleCategory(a))}</span>`:''}</div></div><div class="result-price">${money(a.price)}</div>`;
      div.onclick=()=>{ addItem({nr:a.nr,name:a.name,cat:a.cat,qty:1,price:a.price,priceListId:current.priceListId}); $('articleSearch').value=''; articleListOpen=false; box.innerHTML=''; };
      box.appendChild(div);
    });
  ...
  function fillCategoryFilter(){
    ...
    if(!sel) return;
    const currentValue = selectedCategory || sel.value || '';
    const coll = new Intl.Collator('de-DE', {numeric:true, sensitivity:'base'});
    const cats = Array.from(new Set(articles.map(articleCategory))).filter(Boolean).sort(coll.compare);
    sel.innerHTML = '<option value="">Alle Kategorien</option>' + cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');
    if(cats.includes(currentValue)){ sel.value = currentValue; selectedCategory = currentValue; }
    else { sel.value = ''; selectedCategory = null; }
  }
  function articleCategory(a){
    return String(a?.cat || 'Ohne Kategorie').trim() || 'Ohne Kategorie';
  }
